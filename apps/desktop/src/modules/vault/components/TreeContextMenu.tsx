...
/**
 * Generic floating context menu anchored at (x, y) — used by FolderTree's
 * right-click handler. Closes on outside click, Escape, or any selection.
 *
 * Position is clamped to the viewport so menus opened near the bottom-right
 * edge don't clip off-screen.
 */
export function TreeContextMenu({ x, y, actions, onClose }: Props) {
      ...
      {actions.map((a, i) => {
        ...
        return (
          <button
            ...
            role="menuitem"
            disabled={disabled}
            onClick={() => { if (!disabled) { a.onClick(); onClose(); } }}
