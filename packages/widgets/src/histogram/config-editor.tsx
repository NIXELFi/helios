  ...
  return (
    <div className="flex flex-col gap-1 p-2 text-xs text-helios-text">
      ...
      <label className="flex justify-between"><span>bins</span>
        <input type="number" className="bg-helios-base border border-helios-line px-1 w-32" value={config.bins} onChange={(e) => set("bins", Number(e.target.value))} /></label>
      <label className="flex justify-between"><span>min</span>
        <input type="number" className="bg-helios-base border border-helios-line px-1 w-32" value={config.min ?? ""} onChange={(e) => set("min", e.target.value === "" ? undefined : Number(e.target.value))} /></label>
      <label className="flex justify-between"><span>max</span>
        <input type="number" className="bg-helios-base border border-helios-line px-1 w-32" value={config.max ?? ""} onChange={(e) => set("max", e.target.value === "" ? undefined : Number(e.target.value))} /></label>
      <label className="flex justify-between"><span>color</span>
        <input type="color" className="bg-helios-base border border-helios-line px-1 w-32" value={config.color} onChange={(e) => set("color", e.target.value)} /></label>
