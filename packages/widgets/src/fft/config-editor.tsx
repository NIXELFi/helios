  ...
  return (
    <div className="p-2 text-xs text-helios-text flex flex-col gap-2">
      <label className="flex items-center gap-2">
        <span className="text-[10px] uppercase tracking-wider text-helios-dim w-20">channel</span>
        <ChannelPicker channels={availableChannels} value={config.channelId} onChange={(v) => set("channelId", v)} />
      </label>
      <label className="flex items-center gap-2 text-[11px] cursor-pointer">
        <input type="checkbox" checked={config.windowed} onChange={(e) => set("windowed", e.target.checked)} className="accent-asu-gold" />
        Hanning window
      </label>
      <label className="flex items-center gap-2 text-[11px] cursor-pointer">
        <input type="checkbox" checked={config.useZoomRange} onChange={(e) => set("useZoomRange", e.target.checked)} className="accent-asu-gold" />
        Restrict to current zoom range
      </label>
      <label className="flex items-center gap-2">
        <span className="text-[10px] uppercase tracking-wider text-helios-dim w-20">y scale</span>
      ...
      <label className="flex items-center gap-2">
        <span className="text-[10px] uppercase tracking-wider text-helios-dim w-20">freq scale</span>
      ...
      <label className="flex items-center gap-2">
        <span className="text-[10px] uppercase tracking-wider text-helios-dim w-20">f max</span>
        <input type="number" min={0} value={config.fmaxHz}
               onChange={(e) => set("fmaxHz", Math.max(0, Number(e.target.value)))}
               className="w-20 bg-helios-base border border-helios-line px-1 py-0.5" />
        <span className="text-[10px] text-helios-dim">Hz (0 = auto)</span>
      </label>
    </div>
  );
}
