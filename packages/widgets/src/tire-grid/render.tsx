...
export function TireGridRender(props: WidgetRenderProps<TireGridConfig>) {
  ...
  function corner(c: Corner) {
    const d = data[c];
    return (
      <div className="flex flex-col bg-helios-base border border-helios-line m-1 p-2 flex-1">
        <div className="text-[10px] uppercase text-helios-dim">{c.toUpperCase()}</div>
        <div className="flex-1 flex items-center justify-center">
          <div className="w-full h-2 rounded-sm" style={{ background: tempColor(d.temp) }} />
        </div>
        <div className="font-mono-num text-lg text-helios-text text-center">
          {d.temp === null ? "—" : `${d.temp.toFixed(0)}°`}
        </div>
        <div className="font-mono-num text-xs text-helios-dim text-center">
          {d.pressure === null ? "—" : `${d.pressure.toFixed(1)} psi`}
        </div>
        {config.wearChannels && (
          <div className="mt-1 h-1 bg-helios-line relative">
  ...
  return (
    <div className="w-full h-full bg-helios-panel grid grid-cols-2 grid-rows-2">
