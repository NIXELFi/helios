 ...
 *  Default surface scales are the SDM26-anchored 2026 calibration, each
 *  pinned by its own measurement: lateral 0.574 reproduces the real 5.02 s
 *  skidpad (grip isolated — no aero, no line, no engine); longitudinal 0.70
 *  the ~4.2 s accel. The driven-line effect lives in events.LINE_FACTOR,
 *  not here. These are OUR track-transfer corrections, not tire data. */
export function distillTir(text: string, label: string, scale = 0.574, scaleLong = 0.70): TirGrip {
