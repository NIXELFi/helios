# Physics Agent Loop â€” Design Spec

**Date:** 2026-05-22
**Branch:** `physics-fixes/math-corrections`
**Status:** approved (verbal) â€” revised v3 after spec review v2 (C-1 amendment protocol, C-2 orchestrator mutex, C-3 hooksPath + no-verify policy)
...
The existing branch already closed the obvious silent-wiring bugs and added five opt-in physics levers. This project picks up from there and pushes through the remaining ~12% unrestricted gap to real CBR600 dyno (documented in [two_zone_results.md](../../../physics_findings/audit-2026-05/two_zone_results.md) and [physics_fixes_report.md](../../../physics_findings/audit-2026-05/physics_fixes_report.md) Appendix), audits every config knob against published norms, and extends solver coverage (multi-zone chemistry, quasi-3D port corrections, knock prediction, etc.).
...
- **Replacing the 1D Euler solver core.** Concrete line: any change under `crates/engine-sim/src/solver/` (time-stepping, reconstruction, Riemann solver, limiters themselves) is out-of-scope without explicit user sign-off. New *source terms*, new *closure models*, and new *constitutive relations* anywhere in the codebase are in-scope. If an investigation concludes the solver core must change to make further progress, that finding is logged with status `SOLVER-CHANGE-REQUIRED` and surfaced to the user â€” never auto-implemented.
- **Changing user-facing CFD tab UX in this work-stream.** Internal-only registry + (optional) dashboard.

## 3. Constraints â€” must hold throughout the project
...
**C2. Agent isolation â€” concrete mechanism.** Concurrent investigations run in separate `git worktree` clones with separate `CARGO_TARGET_DIR` paths. Each worktree has its own `Cargo.lock` (worktrees share `.git` but get a fresh checkout). The main checkout is reserved for the orchestrator and the doctor; researchers and implementers only run inside worktrees. Worktree mutation rules in Â§4.5.

**C3. Source-file lock â€” real primitive, declared upfront, with amendment protocol and orchestrator mutex.**

*Lock format.* Before a worktree spawns, the orchestrator declares the investigation's *write claim manifest* â€” a complete list of source files the agent may modify. The manifest is a JSON document at `.physics_locks/NNNN-slug.lock` with fields `{id, slug, worktree_path, spawned_by: cron|manual, spawned_at, files: [...]}`. Locks cover not only `*.rs` source but also `Cargo.toml`, `Cargo.lock`, `crates/engine-sim/src/model/*.rs`, schema files, and `crates/cfd-core/tests/regressions/`. Live locks list their worktree path; stale-lock detection is by missing worktree (cleaned up at orchestrator startup).

*Orchestrator mutex (resolves v2 C-2).* All `.physics_locks/` mutations â€” reading the existing manifests, computing collisions, and writing the new manifest â€” are serialized through `.physics_locks/_orchestrator.mutex`, acquired by `O_EXCL` create. The mutex file holds `{pid, hostname, acquired_at}`. Stale-mutex detection: if `mtime > 120 s` *and* the recorded `pid` is not running, the next orchestrator reclaims it. Both cron-spawned and manual orchestrators use the same mutex â€” there is no orchestrator that bypasses it.
...
**C4. Reproducibility â€” explicit determinism contract.** Every recorded `results.ndjson` is reproducible bit-faithfully on the same `(target_triple, rustc_version, libm_provider)` from `study.toml` + commit hash. To make this true:
...
- Optional bilateral tolerance (`Â±5%`) vs. one-sided (`>= 0.95 * target`).

The skeptic agent reviews the `[acceptance]` block *before the study runs* and acks or challenges the band's literature justification (a pre-run challenge restarts the design phase). After the run, the skeptic checks results strictly against the pre-registered band â€” bands cannot be changed post-hoc except through an explicit "amend acceptance" round counted in the Â§4.3 disagreement loop. If results disagree, the verdict is `NEEDS-FIX`. If literature itself is ambiguous, the finding documents that and may close as `LITERATURE-AMBIGUOUS` (a terminal but explicitly non-validating state).

**C7. Status states â€” finite, terminal, with one re-open path.** States: `INVESTIGATING`, `FIX-IN-PROGRESS`, `VALIDATED`, `FIXED`, `CEILING-LIMIT`, `LITERATURE-AMBIGUOUS`, `SOLVER-CHANGE-REQUIRED`, `ABANDONED`, `STALE`. Terminal states (`VALIDATED`/`FIXED`/`CEILING-LIMIT`/`LITERATURE-AMBIGUOUS`/`SOLVER-CHANGE-REQUIRED`/`ABANDONED`) do not re-open by agent action *except* via the staleness rule (C8). `STALE` is the only re-openable state and is set only by the doctor.

**C8. Staleness â€” baseline fingerprinting, skeptic-validated, two-tier check.** Every finding records a `baseline_fingerprint` block listing content-hashes (SHA-256) of every source file the study transitively reached. The list is researcher-authored but skeptic-validated:
...
- The skeptic checks the fingerprint against the suggested superset as part of VALIDATED/FIXED sign-off (DoD Â§6). A fingerprint that omits files the suggester proposed without written justification fails skeptic review.

*Per-merge fast path vs. periodic full sweep.* On every merge into the investigation branch, the doctor recomputes fingerprints *only for files touched by the merge's diff* and flags any closed finding whose listed files include any diff entry. The full sweep (every closed finding Ã— every listed file) runs once daily or every 25th merge, whichever is sooner â€” this is the authoritative backstop and catches fingerprints that drifted by accident. The fast path is cacheable; the full sweep amortizes.

A `STALE` finding is automatically re-queued for re-validation (status flips back to `INVESTIGATING`). The DoD in Â§7 incorporates this: project complete = "zero non-terminal findings AND zero `STALE` findings."

*STALE re-open lifecycle.* The finding keeps its original `NNNN` id. The worktree dir uses a `revalidation_N` suffix (`worktrees/agent-0042-woschni_revalidation_1/`) so re-runs don't collide with the original. The `finding.md` body grows a `## Revalidations` section that accumulates each pass â€” single audit trail per finding.

*STALE-during-merge.* If during a per-merge doctor sweep the doctor discovers that finding #X *just went STALE because of the merge currently being evaluated for finding #Y*, the #Y merge proceeds (blocking it would deadlock â€” the merge is what caused the staleness). The doctor emits a STALE event into a queue (`physics_findings/_stale_queue.ndjson`) that the orchestrator processes on its next dispatch cycle. Note that #X and #Y cannot have overlapping *write* claims at this point (the lock manifest in C3 prevents that); the staleness arises because #Y wrote a file that #X's fingerprint *read* â€” read dependencies are not lock-protected, only write claims are.
...
- **Mass conservation:** the band is *junction-kind-aware* (amended after finding 0003 diagnosed a characteristic-junction precision floor). Configs are pinned by the `junction` field in `study.toml`; `helios-bench validate` reads it and selects the band.
  - **CV / Stagnation junctions:** `Â± 1e-10` relative per cycle (`|nonconservation_kg| / mass_total_kg â‰¤ 1e-10`). Current behavior on these is machine epsilon ~1e-15 relative; this band allows new source terms â€” heat-transfer with variable density, residual-gas remixing â€” without flagging arithmetic noise as a violation.
  - **Characteristic junctions:** `Â± 5e-4` relative per cycle. Justification: finding 0003 (`physics_findings/0003-conservation-cliff-cycle-15-20/finding.md`) measured a per-cycle leak ceiling of ~3.95e-7 kg on SDM26 / 10000 RPM / 30 cycles (relative ~1.1e-4) and ~6.32e-7 kg at 6000 RPM (relative ~1.75e-4). The cliff is an algorithmic precision floor of the secant-Newton mass-flux residual against the subsequent MUSCL-Hancock face flux â€” parity-locked to `crates/engine-sim/python_ref/`, so a "fix" would break every parity golden. The 5e-4 band accommodates the worst-case measured value with 3Ã— headroom. Any new finding that exceeds it on a characteristic-junction config is a real regression, not algorithmic noise.
  - The actual measured value is recorded per finding so drift is visible regardless of band.
- **Energy conservation:** `Â± 0.5%` per cycle.
- **Momentum conservation:** `Â± 0.5%` per cycle.
...
*Phase reconciliation.* To prevent Phase 1 / Phase 2 stalling on the absence of a second corpus, Phase 0 step 0.2 stands up **SDM25 + SDM26 as the two-calibration baseline.** Both configs already exist in the repo (`crates/engine-sim/python_ref/configs/{sdm25,sdm26}.json`) and model the same physical engine (Honda CBR600RR) but with different solver calibrations (SDM25 is the pre-Phase-F tune; SDM26 is the current). Every tuning fix must improve both calibrations against the same CBR600 dyno target â€” a fix that helps SDM26 hit the dyno but breaks SDM25's prediction reveals coefficient-level over-fit, even though the physical engine is identical. This is a stronger *internal cross-calibration consistency* test than external-engine cross-validation in some respects, because the underlying physics is held constant. Phase 4 still broadens to a truly different engine (FSAE single-cylinder or Heywood Appendix D benchmark) for external cross-engine validation, but Phase 1 / Phase 2 do not block on it. *Bug-fix* findings (e.g., wiring errors like the limiter / junction-loss / AFR-Î· bugs already closed on this branch) are exempt from C10 â€” they fix incorrect behavior and need only the original CBR600 calibration regression to confirm no regression.
...
- **`crates/helios-bench`** â€” new Rust binary. CLI for `run` / `sweep` / `compare` / `validate` / `plot`. Reads `study.toml`, writes NDJSON to a file (first line is the environment block from C4). Deterministic seeds required. Built per-worktree.
- **`crates/helios-mcp`** â€” MCP server wrapping the same crate. Convenience only (C11). Exposes `run_sim`, `submit_sweep`, `read_finding`, `list_findings`, `query_literature`, `validate_results`.
- **`physics_findings/`** â€” registry at repo root:
  - `README.md` â€” auto-generated status board, written *only* by orchestrator
  - `NNN-slug/finding.md` â€” YAML frontmatter (`id`, `status`, `topic`, `hypothesis`, `opened`, `closed`, `owner`, `spawned_by` (`cron`|`manual`), `commit_hash`, `baseline_fingerprint`, `revalidation_count`) + body
  - `NNN-slug/study.toml` â€” reproducible inputs (C4 + C6)
  - `NNN-slug/results.ndjson` â€” raw outputs
  - `NNN-slug/literature.md` â€” citations + equations
  - `PARITY_FLAGS.toml` â€” enumerated parity-preserving default-off flags (C5)
  - `ORCHESTRATOR.md` â€” playbook for the orchestrator (referenced from Â§4.5 step 1)
  - `references/literature/` â€” paraphrased excerpts + equations (with ISBN/edition)
  - `references/dyno/` â€” calibration datasets
- **`worktrees/`** â€” `.gitignore`'d. One subdir per active investigation. Reaped on merge or abandon.
- **`physics_findings/dashboard/`** â€” *deferred* to Phase 5+. Markdown status board is enough until proven otherwise (per spec-review I-5).
- **`.physics_locks/`** â€” JSON write-claim manifests per active investigation (per C3). Committed to `physics-fixes/math-corrections` from the main checkout only (never from a worktree â€” the pre-commit hook refuses). Worktrees see locks read-only through the shared `.git`. Orchestrator writes inside its mutex; spawn-time check fails if any file in a new manifest collides with a live lock.
...
| `physics-skeptic` | Read, Grep, Glob, Bash (sim runs only, via a wrapper command `helios-bench run` â€” Bash policy is enforced by the agent's tool permission list, not by trust), WebFetch, WebSearch | Read-only view of researcher's worktree. Adversarial. |
...
| `physics-doctor` | Read, Bash, Grep | Main checkout. Runs on every merge: full parity suite (C5), full baseline-fingerprint recomputation (C8), full regressions suite. Doctor failure â†’ auto-revert (C1). |
...
*Escalation packet.* When the 3-round cap fires, the orchestrator writes `physics_findings/NNNN-slug/escalation.md` containing the full challenge ledger (every round's challenge + researcher response), the current `study.toml` `[acceptance]` band, and a one-paragraph summary written by the orchestrator of where the disagreement actually lives. The packet â€” not just the latest round â€” is what the user reviews.
...
study.toml (with [acceptance], [environment], seed)
        â”‚
        â–¼
helios-bench run â”€â–º results.ndjson (first line = environment block)
        â”‚                  â”‚
        â–¼                  â–¼
   pre-commit          validate (invariants per C9)
   parity gate (C5)         â”‚
        â”‚                  â–¼
        â–¼            literature.md â—„â”€â”€ references/literature/*.md
   compare(baseline)       â”‚             references/dyno/*.csv
        â”‚                  â–¼                      â–²
        â–¼            finding.md (with             â”‚
   commit             baseline_fingerprint)    WebFetch/WebSearch
        â”‚                  â”‚
        â–¼                  â–¼
   doctor sweep â”€â”€â”€â”€â–º  merge (or auto-revert on doctor fail)
```

### 4.5 Investigation lifecycle

1. **Orchestrator picks finding.** On each dispatch cycle, the orchestrator (a) processes any `pending_amend.json` from active worktrees (per C3 amendment protocol â€” keeps blocked researchers from starving), (b) drains the `_stale_queue.ndjson` re-opening any STALE findings, then (c) picks the next `INVESTIGATING` / `STALE` / new candidate from the priority queue. Reads `physics_findings/ORCHESTRATOR.md` playbook + `README.md` status board. Drafts `study.toml` skeleton with `[acceptance]` and the *write-claim manifest* listing every source file the agent may touch.
2. **Spawn worktree.** Orchestrator creates `worktrees/agent-NNNN-slug/` via `git worktree add`. Lock manifest written to `.physics_locks/NNNN-slug.lock` (commit on main checkout). If any file in manifest is already locked, spawn rejected â†’ queue. CARGO_TARGET_DIR is the worktree's own `target/`.
...
4. **Skeptic review (pre-fix path).** Read-only view of worktree. Checks pre-registered acceptance band, comparison-class correctness, missed conservation, literature consistency. Outputs structured APPROVE or CHALLENGE (the latter includes falsification tests per Â§4.3).
5. **Branch by verdict:**
   - **VALIDATED** (within acceptance band, no fix needed): researcher records `baseline_fingerprint`. Pre-commit gate (parity + manifest) runs. Commit. Doctor sweep. On doctor green â†’ merge to investigation branch â†’ reap worktree â†’ release lock.
   - **NEEDS-FIX**: implementer in same worktree adds the fix + regression test. Researcher re-runs study. Skeptic re-reviews (round counter incremented). On agreement â†’ record `baseline_fingerprint` â†’ pre-commit gate â†’ commit â†’ doctor sweep â†’ merge â†’ reap â†’ release.
...
- Original correlation papers: Woschni 1967, Chen-Flynn 1965, Engelman 1973, Annand 1963 â€” pinned by DOI when available
...
- **0.2** `physics_findings/` scaffold + first reference docs (Heywood + Woschni + Chen-Flynn paraphrases) + `PARITY_FLAGS.toml` + `ORCHESTRATOR.md` + **second-engine corpus** (FSAE single-cylinder or Heywood Appendix D â€” required by C10)
...
- **0.A** End-to-end smoke test: reproduce one of the existing parity goldens (suggest `engine_matrix_sdm26_baseline.json`) bit-exactly through `helios-bench run`, then run the full investigation lifecycle (orchestrator â†’ researcher â†’ skeptic â†’ implementer if needed â†’ doctor â†’ merge) on a trivial known finding (suggest the already-VALIDATED limiter bug fix) to prove every piece of plumbing works end-to-end.

### Phase 1 â€” Seeded investigations
...
3. Variable Î³ + dissociation chemistry (extends two-zone model)
4. Turbulent burn-rate correlation (refines tumble factor)
5. T_b clamp at 3500 K â€” verify or lift
...
### Phase 2 â€” Broad-front parameter audit
...
Estimated 50â€“100 new findings spawned here.

### Phase 3 â€” Solver-class extensions (inside the 1D framework only â€” see Â§2)

Each is a multi-investigation track:
- Multi-zone (â‰¥3) combustion with NASA-7 equilibrium chemistry
- Variable Î³(T, composition)
...
### Phase 4 â€” Continuous calibration + multi-engine cross-validation

- Scheduled overnight autonomous runs via the `schedule` skill (cron mechanism â€” chosen for predictable lock-ledger semantics: cron-spawned orchestrators serialize with manual ones through the same git pre-commit + lock acquisition path)
- Calibration regression suite â€” any merged fix re-runs full CBR600 dyno comparison
- Cross-validate against â‰¥2 additional reference engines (already required per C10 for any tuning fix; Phase 4 is the broad sweep)
...
### Phase 5 â€” Documentation & summary
...
## 6. Definition of Done â€” per finding
...
- Parity suite passes (C5) â€” checked at pre-commit and at doctor merge
...
## 7. Definition of Done â€” for the project
...
- Zero `STALE` findings remain (per C8 â€” every fingerprint up-to-date)
...
| Risk | Mitigation |
|---|---|
| Agent slop science | Adversarial skeptic; structured `challenge.md` (Â§4.3); pre-registered `[acceptance]` (C6); offline literature corpus; doctor sweeps |
...
| Concurrent regression-test names | Test filename includes finding id (`r0042_woschni_c1_c2.rs`) â€” collision impossible |
...
| Infinite skeptic/researcher loop | 3-round cap â†’ auto-escalate to user (Â§4.3) |
| Calibration over-fit to CBR600 | C10: tuning fix requires â‰¥2-engine validation before `FIXED` |
...
## 10. References â€” required reading for any contributor

- [physics_synthesis.md](../../../physics_findings/audit-2026-05/physics_synthesis.md)
- [physics_fixes_report.md](../../../physics_findings/audit-2026-05/physics_fixes_report.md)
- [physics_validation_report.md](../../../physics_findings/audit-2026-05/physics_validation_report.md) (skim)
- [two_zone_results.md](../../../physics_findings/audit-2026-05/two_zone_results.md)
- [conservation_audit_report.md](../../../physics_findings/audit-2026-05/conservation_audit_report.md)
- [parity_offnominal_report.md](../../../physics_findings/audit-2026-05/parity_offnominal_report.md)
