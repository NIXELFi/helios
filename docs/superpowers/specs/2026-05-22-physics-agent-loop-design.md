...
- **Mass conservation:** the band is *junction-kind-aware* (amended after finding 0003 diagnosed a characteristic-junction precision floor). Configs are pinned by the `junction` field in `study.toml`; `helios-bench validate` reads it and selects the band.
  - **CV / Stagnation junctions:** `± 1e-10` relative per cycle (`|nonconservation_kg| / mass_total_kg ≤ 1e-10`). Current behavior on these is machine epsilon ~1e-15 relative; this band allows new source terms — heat-transfer with variable density, residual-gas remixing — without flagging arithmetic noise as a violation.
  - **Characteristic junctions:** `± 5e-4` relative per cycle. Justification: finding 0003 (`physics_findings/0003-conservation-cliff-cycle-15-20/finding.md`) measured a per-cycle leak ceiling of ~3.95e-7 kg on SDM26 / 10000 RPM / 30 cycles (relative ~1.1e-4) and ~6.32e-7 kg at 6000 RPM (relative ~1.75e-4). The cliff is an algorithmic precision floor of the secant-Newton mass-flux residual against the subsequent MUSCL-Hancock face flux — parity-locked to `crates/engine-sim/python_ref/`, so a "fix" would break every parity golden. The 5e-4 band accommodates the worst-case measured value with 3× headroom. Any new finding that exceeds it on a characteristic-junction config is a real regression, not algorithmic noise.
