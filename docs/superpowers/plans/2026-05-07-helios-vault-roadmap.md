...
|---|---|---|---|---|---|
| 1 | **Backend foundation** | `2026-05-07-helios-vault-1-backend.md` | not started | — | A live Supabase project with the `pdm` schema, RLS policies, RPCs, audit log, and bootstrap-admin script. Manually testable end-to-end via SQL or `supabase-js`: an admin can create a vault, an editor can acquire/release a lock, a non-holder cannot insert a version, an admin can force-unlock. |
