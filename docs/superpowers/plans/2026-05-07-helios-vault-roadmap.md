...
| 4 | **Vault module UI** | `2026-05-07-helios-vault-4-vault-ui.md` | code complete @ db5bdf5 | Plan 3 | Inside Helios, the Vault module gains real screens: Browse (folder tree + file table with live lock state via Supabase Realtime), File detail (history, refs, audit), History viewer, Search, Who-has-what, Admin → Users, Admin → Vaults, Settings. Read paths and admin operations are functional; check-in/out from the UI is stubbed (real check-in/out arrives in Plan 6 when the daemon exists). |
